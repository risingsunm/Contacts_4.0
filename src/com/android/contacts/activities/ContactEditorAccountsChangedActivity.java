...
public class ContactEditorAccountsChangedActivity extends Activity {
    ...
    public int getGSMSimCapacity(){
        ...
        try {
            ...
            if (iccIpb != null) {
                adnRecords = iccIpb.getAdnRecordsInEfOnSubscription(IccConstants.EF_ADN, 1);
            }
        } catch (RemoteException ex) {
            // ignore it
        } catch (SecurityException ex) {
            if (false) Log.e(TAG, ex.toString());
        }
    ...
    public int getCDMASimCapacity(){
        ...
        try {
            IIccPhoneBookMSim iccIpb = IIccPhoneBookMSim.Stub.asInterface(
                    ServiceManager.getService("simphonebook_msim"));
            if (iccIpb != null) {
                adnRecords = iccIpb.getAdnRecordsInEfOnSubscription(IccConstants.EF_ADN, 0);
            }
        } catch (RemoteException ex) {
            // ignore it
        } catch (SecurityException ex) {
    ...
    private final OnItemClickListener mAccountListItemClickListener = new OnItemClickListener() {
        ...
        public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
            ...
            String mChoiceAccount = mAccountListAdapter.getItem(position).name;
            if(mChoiceAccount.equals("PHONE")){
                if(getPhoneContactsTotalCount()<MAX_CONTACT_TOTAL_COUNT){
                    saveAccountAndReturnResult(mAccountListAdapter.getItem(position));
                }else{
                    Toast.makeText(getApplicationContext(), R.string.out_of_phone_capacity, Toast.LENGTH_LONG).show();
                    finish();
                }
            }else if(mChoiceAccount.equals("SIM1")){
                if(getCDMASimContactTotalCount()<getCDMASimCapacity()){
                    saveAccountAndReturnResult(mAccountListAdapter.getItem(position));
                }else {
                    Toast.makeText(getApplicationContext(), R.string.out_of_sim1_capacity, Toast.LENGTH_LONG).show();
                    finish();
                }
            }else if(mChoiceAccount.equals("SIM2")){
                if(getGSMSimContactTotalCount()<getGSMSimCapacity()){
                    saveAccountAndReturnResult(mAccountListAdapter.getItem(position));
                }else {
                    Toast.makeText(getApplicationContext(), R.string.out_of_sim2_capacity, Toast.LENGTH_LONG).show();
                    finish();
                }
            }else{
                    saveAccountAndReturnResult(mAccountListAdapter.getItem(position));
