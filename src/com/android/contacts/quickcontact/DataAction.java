...
public class DataAction implements Action {
     ...
     */
    /*Begin: Modified by wqiang for QuickContact 2012/04/12*/
    public DataAction(int index, Context context, String mimeType, DataKind kind, long dataId, Cursor cursor) {
        mContext = context;
        mKind = kind;
        mMimeType = mimeType;
        mindex = index;
        ...
        if (Phone.CONTENT_ITEM_TYPE.equals(mimeType)) {
            if (PhoneCapabilityTester.isPhone(mContext)) {
                ...
                if (!TextUtils.isEmpty(number)) {
                    ...
                    phoneIntent.putExtra("subscription", 0);
            ...
            if (Phone.CONTENT_ITEM_TYPE.equals(mimeType)) {
                if (PhoneCapabilityTester.isPhone(mContext)) {
                    ...
                    if (!TextUtils.isEmpty(number)) {
                        ...
                        final Intent phoneIntent = hasPhone ? new Intent(Intent.ACTION_CALL_PRIVILEGED,
                                Uri.fromParts(Constants.SCHEME_TEL, number, null)) : null;
                        phoneIntent.putExtra("subscription", 1);
                    phoneIntent.putExtra("directDial", true);
                        final Intent smsIntent = hasSms ? new Intent(Intent.ACTION_SENDTO,
                                Uri.fromParts(Constants.SCHEME_SMSTO, number, null)) : null;
            ...
            if (Phone.CONTENT_ITEM_TYPE.equals(mimeType)) {
                if (PhoneCapabilityTester.isPhone(mContext)) {
                    ...
                    if (!TextUtils.isEmpty(number)) {
                        final Intent phoneIntent = hasPhone ? new Intent(Intent.ACTION_CALL_PRIVILEGED,
                                Uri.fromParts(Constants.SCHEME_TEL, number, null)) : null;
                        phoneIntent.putExtra("ipcall", true);
                        if (hasPhone) {
                            mIntent = phoneIntent;
                        }
                    }
