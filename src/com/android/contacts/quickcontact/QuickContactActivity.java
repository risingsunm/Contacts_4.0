...
public class QuickContactActivity extends Activity {
    ...
    /*Begin: Modified by wqiang for QuickContact 2012/04/12*/
    private final int CALL_C = 0;
    private final int CALL_G = 1;
    private final int CALL_IP = 2;
    private final int EMAIL = 3;
    private final int WEBSITE = 4;
    /*End: Modified by wqiang for QuickContact 2012/04/12*/
    ...
    /*Begin: Modified by wqiang for QuickContact 2012/04/12*/
    private List<String> mSortedActionMimeType = Lists.newArrayList();
    /*End: Modified by wqiang for QuickContact 2012/04/12*/
    ...
    /*Begin: Modified by wqiang for QuickContact 2012/04/12*/
    private AHongActionMultiMap aHongActions = new AHongActionMultiMap();
    /*End: Modified by wqiang for QuickContact 2012/04/12*/
    ...
    private void bindData(Cursor cursor) {
        ...
        /*Begin: Modified by bxinchun, Change the quickcontact dialog layout 2012/07/25*/
        /*mOpenDetailsButton.setVisibility(isMimeExcluded(Contacts.CONTENT_ITEM_TYPE) ? View.GONE
                : View.VISIBLE);*/
        mOpenDetailsButton.setVisibility(View.GONE);
        /*End: Modified by bxinchun, Change the quickcontact dialog layout 2012/07/25*/
        ...
        /*Begin: Modified by wqiang for QuickContact 2012/04/12*/
        for (List<Action> actionChildren : aHongActions.values()) {
            Collapser.collapseList(actionChildren);
        }
        /*End: Modified by wqiang for QuickContact 2012/04/12*/
        ...
        if (photoView != null) {
            ...
            if (photoBitmap != null) {
                ...
                /*Begin: Modified by bxinchun for setting default contact icon 2012/07/25*/
                //photoView.setImageResource(ContactPhotoManager.getDefaultAvatarResId(true, false));

                long contactId = 0;
                if (!cursor.isNull(DataQuery.RAW_CONTACT_ID) && cursor.moveToFirst()) {
                    contactId = cursor.getLong(DataQuery.RAW_CONTACT_ID);
                }
                photoView.setImageResource(ContactPhotoManager.getDefaultAvatarResId(contactId));
                /*End: Modified by bxinchun for setting default contact icon 2012/07/25*/
            }
        }
        ...
        for (String mimeType : mSortedActionMimeTypes) {
            if(Phone.CONTENT_ITEM_TYPE.equals(mimeType)){
                if(!TelephonyManager.getTelephonyProperty("gsm.sim.state",0,"").equals("ABSENT")||(TelephonyManager.getTelephonyProperty("gsm.sim.state",0,"").equals("ABSENT")&&TelephonyManager.getTelephonyProperty("gsm.sim.state",1,"").equals("ABSENT")))
                   mSortedActionMimeType.add(mimeType);
            ...
            else if(Website.CONTENT_ITEM_TYPE.equals(mimeType)||Email.CONTENT_ITEM_TYPE.equals(mimeType)){
                mSortedActionMimeType.add(mimeType);
            }
        }
        /*End: Modified by wqiang for QuickContact 2012/04/12*/
        ...
        /*Begin: Modified by wqiang for QuickContact 2012/04/12*/
        for (int index=0;index<mSortedActionMimeType.size();index++){
            cursor.moveToFirst();
            String mimeType = mSortedActionMimeType.get(index);
                do {
                    // Handle any social status updates from this row
                    status.possibleUpdate(cursor);

                    final String mimeType1 = cursor.getString(DataQuery.MIMETYPE);

                    // Skip this data item if MIME-type excluded
                    if (isMimeExcluded(mimeType1)) continue;
                    ...
                    // Handle photos included as data row
                    if (Photo.CONTENT_ITEM_TYPE.equals(mimeType1)) {
                        final int displayPhotoColumnIndex = cursor.getColumnIndex(Photo.PHOTO_FILE_ID);
                        final boolean hasDisplayPhoto = !cursor.isNull(displayPhotoColumnIndex);
                        if (hasDisplayPhoto) {
                            final long displayPhotoId = cursor.getLong(displayPhotoColumnIndex);
                    ...
                    if (kind != null) {
                        ...
                        // element, build its summary from the cursor, and collect it
                        // along with all others of this MIME-type.
                        if(Phone.CONTENT_ITEM_TYPE.equals(mimeType1)&&index<3){
                            final Action action = new DataAction(index, context, mimeType1, kind, dataId, cursor);
                            aHongActions.put(index, action);
                            continue;
                        }
                    }
                    ...
                    // Handle Email rows with presence data as Im entry
                    final boolean hasPresence = !cursor.isNull(DataQuery.PRESENCE);
                    if (hasPresence && Email.CONTENT_ITEM_TYPE.equals(mimeType1)) {
                        final DataKind imKind = accountTypes.getKindOrFallback(accountType, dataSet,
                                Im.CONTENT_ITEM_TYPE);
                        if (imKind != null) {
                            final DataAction action = new DataAction(context, Im.CONTENT_ITEM_TYPE, imKind,
            ...
            final View actionView = inflateAction(index,mimeType, cache, mTrack);
             mTrack.addView(actionView);
        }
        /*End: Modified by wqiang for QuickContact 2012/04/12*/
        ...
        final boolean hasData = !mSortedActionMimeType.isEmpty();
        /*End: Modified by wqiang for QuickContact 2012/04/12*/
...
//
    //    return typeView;
    //}
    private View inflateAction(int index, String mimeType, ResolveCache resolveCache, ViewGroup root) {
        final CheckableImageView typeView = (CheckableImageView) getLayoutInflater().inflate(
                R.layout.quickcontact_track_button, root, false);
        List<Action> children = aHongActions.get(index);
        typeView.setIndex(index);
        typeView.setTag(mimeType);
        if(!TelephonyManager.getTelephonyProperty("gsm.sim.state",0,"").equals("ABSENT")&&TelephonyManager.getTelephonyProperty("gsm.sim.state",1,"").equals("ABSENT")){
            if(index>=1)
                index++;
        ...
        switch (index) {
        case CALL_C:
            icon=getResources().getDrawable(R.drawable.call_c);
            break;
        case CALL_G:
            icon=getResources().getDrawable(R.drawable.call_g);
            break;
        case CALL_IP:
            icon=getResources().getDrawable(R.drawable.call_ip);
            break;
        case EMAIL:
            icon=getResources().getDrawable(R.drawable.email);
            break;
        case WEBSITE:
            icon=getResources().getDrawable(R.drawable.website);
            break;
        default:
            break;
        }
    ...
    private class ViewPagerAdapter extends FragmentPagerAdapter {
        ...
        public Fragment getItem(int position) {
            QuickContactListFragment fragment = new QuickContactListFragment();
            final String mimeType = mSortedActionMimeType.get(position);
            if(Phone.CONTENT_ITEM_TYPE.equals(mimeType)&&position<3){
                  final List<Action> actions = aHongActions.get(position);
                  fragment.setActions(actions);
            }else{
                final List<Action> actions1 = mActions.get(mimeType);
                fragment.setActions(actions1);
            }
            return fragment;
        }
        /*End: Modified by wqiang for QuickContact 2012/04/12*/
        ...
        @Override
        public int getCount() {
            return mSortedActionMimeType.size();
        }
        /*End: Modified by wqiang for QuickContact 2012/04/12*/
    }
    ...
    private interface DataQuery {
        ...
        final int RAW_CONTACT_ID = 16; // add by bxinchun 2012-07-25
    }
}
