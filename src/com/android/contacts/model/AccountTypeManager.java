    ...
    public List<AccountWithDataSet> getAccounts(boolean contactWritableOnly,
        ...
        switch (flag) {
            ...
            case FLAG_ALL_ACCOUNTS_WITHOUT_LOCAL:
                List<AccountWithDataSet> tempListLocal = Lists.newArrayList();
                if (contactWritableOnly) {
                    for (AccountWithDataSet accountWithDataSet : mContactWritableAccounts) {
                        if (!SimAccountType.ACCOUNT_TYPE
                                .equals(accountWithDataSet.type)
                                && !PhoneAccountType.ACCOUNT_TYPE
                                        .equals(accountWithDataSet.type)) {
                            tempListLocal.add(accountWithDataSet);
                        }
                    }
                } else {
                    for (AccountWithDataSet accountWithDataSet : mAccounts) {
                        if (!SimAccountType.ACCOUNT_TYPE
                                .equals(accountWithDataSet.type)
                                && !PhoneAccountType.ACCOUNT_TYPE
                                        .equals(accountWithDataSet.type)) {
