...
public abstract class ContactListAdapter extends ContactEntryListAdapter {
    ...
    protected static class ContactQuery {
        ...
        private static final String[] FILTER_PROJECTION_PRIMARY = new String[] {
            ...
            /*Begin: Modified by siliangqi for contact_searchresult_color 2012-7-24*/
            "short_name_normal",
            "full_name_normal",
            "hanzi_name",
            /*End: Modified by siliangqi for contact_searchresult_color 2012-7-24*/
        };
    ...
    public int getContactId(int position) {
        return ((Cursor) getItem(position)).getInt(ContactQuery.CONTACT_ID);
    }
    ...
    /**
     * to do sql operation async to avoid ANR
     */
    protected void bindSimPhoto(ImageView view, Cursor cursor) {
        long contactId = cursor.getLong(MultiPickContactActivity.SUMMARY_ID_COLUMN_INDEX);
        Pair<View, Object> pair = new Pair<View, Object>(view, null);
        Log.d(TAG, "start sim query,contact id=" + contactId);
        ContactPhotoManager.getInstance(mContext).new SimQueryHandler(mContext)
                .startQuery(QUERY_SIM_TOKEN, pair,
                        RawContacts.CONTENT_URI,
                        SimContactsOperation.ACCOUNT_PROJECTION, RawContacts.CONTACT_ID + "="
    ...
    protected void bindPhoto(final ContactListItemView view, int partitionIndex, Cursor cursor) {
        ...
        if (photoId != 0) {
            ...
            if (photoUri == null) {
                long contactId = 0;
                if (!cursor.isNull(ContactQuery.CONTACT_ID)) {
                    contactId = cursor.getLong(ContactQuery.CONTACT_ID);
                }
                getPhotoLoader().loadPhoto(view.getPhotoView(), photoUri, false, false, contactId);
            } else {
                getPhotoLoader().loadPhoto(view.getPhotoView(), photoUri, false, false);
            }
