...
public class DefaultContactListAdapter extends ContactListAdapter {
    ...
    public DefaultContactListAdapter(Activity target) {
        super(target);
        this.target = target;
    }
    ...
    private void configureSelection(
        ...
        if(Intent.ACTION_ATTACH_DATA.equals(target.getIntent().getStringExtra("super_action"))){
            selection.append(RawContacts.ACCOUNT_TYPE + "!=?");
            selectionArgs.add(SimAccountType.ACCOUNT_TYPE);
    ...
    protected void bindView(View itemView, int partition, Cursor cursor, int position) {
        ...
        if (isQuickContactEnabled()) {
            ...
            //bindSimPhoto(view.getQuickContact(),cursor);

            /*Begin: Added by bxinchun for showing sim mark view 2012/07/19*/
            bindSimPhoto(view.getSimMarkView(), cursor);
            /*End: Added by bxinchun for showing sim mark view 2012/07/19*/
        } else {
            bindPhoto(view, partition, cursor);
            //bindSimPhoto(view.getPhotoView(),cursor);
