...
public final class AccountsListAdapter extends BaseAdapter {
    ...
    private List<AccountWithDataSet> getAccounts(AccountListFilter accountListFilter) {
        ...
        final List<AccountWithDataSet> writableAccountList = mAccountTypes.getAccounts(
        ...
        } else if (accountListFilter ==
            AccountListFilter.ACCOUNTS_CONTACT_WRITABLE_WITHOUT_PHONE) {
            for (AccountWithDataSet account: writableAccountList) {
                    if (SimContactsConstants.ACCOUNT_TYPE_PHONE.equals(account.type))
                        deletedList.add(account);
            }
            writableAccountList.removeAll(deletedList);
        }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        /*Begin: Modified by sunrise for NewContactsDialog 2012/07/24*/
        System.out.println(account.toString());
        //icon.setImageDrawable(accountType.getDisplayIcon(mContext));
        if (account.name.contains("SIM1"))
        {
            icon.setImageResource(R.drawable.ic_contact_list_sim1);
        }
        else if (account.name.contains("SIM2"))
        {
            icon.setImageResource(R.drawable.ic_contact_list_sim2);
        }
        else
        {
            icon.setImageResource(R.drawable.ic_launcher_phone);
        }
        /*End: Modified by sunrise for NewContactsDialog 2012/07/24*/
