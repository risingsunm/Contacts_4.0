...
/*Begin: Modified by xiepengfei for query contact info 2012/04/06*/
///*package*/ class CallLogQueryHandler extends AsyncQueryHandler {
public class CallLogQueryHandler extends AsyncQueryHandler {
/*End: Modified by xiepengfei for query contact info 2012/04/06*/
    ...
    /*Begin: Modified by xiepengfei for  2012/03/23*/
    public void fetchContactByLookupUri(String s){
        cancelFetch();
        invalidate();
        fetchCallsContact(QUERY_NEW_CALLS_TOKEN,true,false,s);
        fetchCallsContact(QUERY_OLD_CALLS_TOKEN,false,false,s);
    }
    public void fetchCallsContact(int token,boolean isNew, boolean voicemailOnly,String s){

        /*Begin: Modified by xiepengfei for calls query bug 2012/06/04*/
        String selection = String.format("%s IS NOT NULL AND %s = 0 AND %s > ?",
                Calls.IS_READ, Calls.IS_READ, Calls.DATE);
        /*End: Modified by xiepengfei for calls query bug 2012/06/04*/
        ...
        selection = selection +" and lookup_uri =  '"+s+"'";
        Log.v(TAG,"Eden selection is :"+selection);
        if (voicemailOnly) {
            // Add a clause to fetch only items of type voicemail.
            selection = String.format("(%s) AND (%s = ?)", selection, Calls.TYPE);
            selectionArgs.add(Integer.toString(Calls.VOICEMAIL_TYPE));
    ...
    /*End: Modified by xiepengfei for  2012/03/23*/
    ...
    public boolean hasCallLog() {
        ...
        Cursor c = null;
        try {
            c = mContentResolver.query(Calls.CONTENT_URI, null, null, null, null);
