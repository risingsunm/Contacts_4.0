...
public abstract class IntentProvider {
    ...
    /*Begin: Add by bxinchun 2012-08-13, add get history item clicked intent*/
    public static IntentProvider getCallDetailIntentProvider(
            final String voicemailUri, final long[] ids, final int subscription) {
        return new IntentProvider() {
            @Override
            public Intent getIntent(Context context) {
                Intent intent = new Intent(context, CallDetailActivity.class);
                ...
                if (ids.length > 1) {
                    // We want to restore the position in the array at the end.
                    intent.putExtra(CallDetailActivity.EXTRA_CALL_LOG_IDS, ids);
                } else {
                    // If there is a single item, use the direct URI for it.
                    intent.setData(ContentUris.withAppendedId(
                            Calls.CONTENT_URI_WITH_VOICEMAIL, ids[0]));
                }
                intent.putExtra(MSimConstants.SUBSCRIPTION_KEY, subscription);
                return intent;
            }
    ...
    /*End: Add by bxinchun 2012-08-13, add get history item clicked intent*/
}
