 ...
 * the category instead of scrolling the list. In order to use this control,
 * your adapter must implement the Indexer interface, then if you want the thumb
 * synchronize when the list scrolling, you must implement the onScroll function
 * to calculate the current section and set it to BladeView, so the BladeView
 * will redraw itself to perform synchronization. As an blade view, the full
 * sections and replaced section should set before drawing process start, you
 * can either set it in xml file as attributes set or call functions, we don't
 * judge whether it is valid, so user need to pay attention, same with section
...
public class BladeView extends View {
    ...
    private static final int DEFAULT_SECTION_ENABLE_COLOR = Color.argb(0xFF, 0x00, 0xA2, 0xFF);
    private static final int DEFAULT_SECTION_DISABLE_COLOR = Color.argb(0xFF, 0xA5, 0xA5, 0xA5);
    private static final int DEFAULT_PROMPT_ENABLE_COLOR = Color.argb(0xFF, 0x00, 0xA2, 0xFF);
    private static final int DEFAULT_PROMPT_DISABLE_COLOR = Color.argb(0xFF, 0x00, 0xA2, 0xFF);
    ...
    /*
     * Contains full sections, but some of its elements are replaced with dashes
     * to save space.
     */
    ...
    /*
     * When it is an header-footer list, represents the count of headers, 0 in
     * normal list.
     */
    ...
    /*
     * A boolean array used to indicate whether the section is in the list, true
     * if absent.
     */
    private boolean[] mIsAbsent;

    /*
     * Use enable color and disable color to distinguish sections present and
     * absent.
     */
    private Paint mEnableSectionPaint;
    private int mEnablePaintColor;
    private Paint CurrentSectionPaint;
    ...
    /*
     * Indicator drawable overlay the current section to prompt user whether
     * he/she is.
     */
    ...
    /*
     * In order to optimize the drawing time, we only draw the dirty area of the
     * view, mRedrawRect restore information of the area.
     */
    ...
    /*
     * When handling touch event, block the setCurrentSection message sent by
     * client.
     */
    ...
    public BladeView(Context context, AttributeSet attrs, int defStyle) {
        ...
        mAnimationDuration = a.getInt(R.styleable.BladeView_promptAnimationDuration,
                PROMPT_ANIM_DURATION);
    ...
    private void init(Context context) {
        ...
        CurrentSectionPaint = new Paint(mEnableSectionPaint);
        CurrentSectionPaint.setColor(DEFAULT_PROMPT_ENABLE_COLOR);
        ...
        mEnterAnim = new PromptRotateAnimation(-PROMPT_HIDE_ANGLE, PROMPT_SHOW_ANGLE,
                promptWidth >> 1);
        mEnterAnim.setDuration(mAnimationDuration);
        mEnterAnim.setFillAfter(true);
        mEnterAnim.setInterpolator(new LinearInterpolator());

        mExitAnim = new PromptRotateAnimation(PROMPT_SHOW_ANGLE, PROMPT_HIDE_ANGLE,
                promptWidth >> 1);
    ...
    public void onDraw(Canvas canvas) {
        ...
        /*
         * Draw replaced sections, different color indicates whether the section
         * is present.
         */
        ...
        for (int i = 0; i < mFullSectionLen; i++) {
            ...
            if(!bg_flag){
                ...
                if(mCurrentSection==i){
                    canvas.drawText(mFullSections[i], centerWidth,
                        mSectionBaselines[i], CurrentSectionPaint);
                    continue;
                }
                canvas.drawText(mFullSections[i], centerWidth,
    ...
    Handler mHander = new Handler(){
        ...
        public void handleMessage(Message msg) {
            ...
            bg_flag = false;
            setBackgroundColor(Color.argb(0x00, 00, 00, 00));
        }
    };
    @Override
    public boolean onTouchEvent(MotionEvent ev) {
        /*
         * If there is no list related, use the default touch event handling
         * process.
         */
        if (mList == null) {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                ...
                if(t!=null)
                    t.interrupt();
                bg_flag = true;
                setBackgroundResource(R.drawable.bladeview_bg);
                if (mListAdapter == null && mList != null) {
                    getSectionsFromIndexer();
                }

                /*
                 * Records the old section position and the calculate the
                ...
                /*
                 * Invalidate the invalid rectangle to do self redraw process if
                 * needed.
                 */
                redrawIfNeeded();
                break;
            ...
            case MotionEvent.ACTION_MOVE:
                /*
                 * Judge whether current motion position and the last motion
                 * position are in the same slot.
                 */
                if (y == mLastMotionY) {
                    return true;
                    ...
                    for (int i = 1; i < mFullSectionLen; i++) {
                        if (y >= mSectionBaselines[i - 1] && y < mSectionBaselines[i]
                                && mLastMotionY >= mSectionBaselines[i - 1]
                                && mLastMotionY < mSectionBaselines[i]) {
                            return true;
                        }
                    }
                }
     ...
     * @return The index in the full sections, revise it when the result is out
     *         of bounds.
     */
    ...
    private void moveListToSection(final int fullSection) {
        ...
        /*
         * Add mListOffset for all list view, because it will be 0 if it has no
         * header.
         */
     ...
     * @return The index in full section(mOriginSections), if there is not
     *         return -1.
     */
    ...
    /**
     * Translate full section index(mOriginSections) to position in list
     * section.
     *
    ...
    private void redrawIfNeeded() {
        ...
        /*
         * Optimized, redraw parts of itself only if the thumb changes its
         * position.
         */
    ...
    public void setSections(final String[] fullSectionArray, final String[] replacedSectionArray) {
        if (fullSectionArray == null || replacedSectionArray == null) {
            throw new InvalidParameterException(
                    "Origin sections and replaced section should not be a null pointer!");
        }

        if (fullSectionArray.length != replacedSectionArray.length) {
            throw new InvalidParameterException(
                    "The length of origin and replaced sections should be equal !");
        }
    ...
    /**
     * Set current index to the specified sectionIndex, this will be called
     * after the related list scrolled, the indicator will also update it
     * position if needed.
     *
    ...
    /**
     * Set the related list view and get indexer information from it, every time
     * when you change the data of list, you should call it to reset the list.
     *
    ...
    /**
     * Set absent section array, if the section in full sections is not in the
     * list sections, set absent flag to true, otherwise false.
     */
    ...
    /**
     * Start the window exit animation, and then dismiss the pop up window after
    ...
    private void positionPromptWindow() {
        ...
        /*
         * When touch event move out of the view, mHintText becomes null causes
         * window leak.
         */
        if (mPromptText == null) {
            return;
        ...
        /*
         * Show the prompt window or update its position if it is already
         * visible.
         */
